      ...
      <div style={{ maxWidth: '800px', margin: '0 auto' }}>
        ...
        {!bufferKey ? (
          <div style={{ backgroundColor: '#1e293b', padding: '20px', borderRadius: '12px', border: '1px solid #334155', marginBottom: '20px' }}>
            <h3 style={{ marginTop: 0 }}>🔑 مفتاح الربط ببافر</h3>
            <input
              type="text"
              value={bufferKey}
              onChange={(e) => setBufferKey(e.target.value)}
              placeholder="الصق مفتاح Buffer API هنا"
              style={{ width: '100%', padding: '12px', borderRadius: '8px', border: '1px solid #334155', backgroundColor: '#0f172a', color: 'white', fontSize: '14px', boxSizing: 'border-box', marginBottom: '10px' }}
            />
            <button
              type="button"
              onClick={loadChannelsManually}
              disabled={isLoadingChannels}
              style={{ padding: '10px 20px', borderRadius: '8px', border: 'none', backgroundColor: '#0ea5e9', color: 'white', fontWeight: 'bold', cursor: 'pointer' }}
            >
              {isLoadingChannels ? '⏳ جاري تحميل القنوات...' : '🔄 تحميل/تحديث قائمة المنصات'}
            </button>
            {channelLoadError && (
              <p style={{ color: '#fbbf24', fontSize: '13px', marginTop: '10px', marginBottom: 0 }}>{channelLoadError}</p>
            )}
          </div>
        ) : (
          <>
